...
const ROLES = [
  { id: 'farmer',     icon: faSeedling, labelKey: 'role_farmer',     descKey: 'role_farmer_desc' },
  { id: 'merchant',   icon: faStore,    labelKey: 'role_merchant',   descKey: 'role_merchant_desc' },
  { id: 'transporter',icon: faTruck,    labelKey: 'role_transporter',descKey: 'role_transporter_desc' },
];
...
export default function AuthPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', password: '', region: REGIONS[0] });
  ...
  const DEMO = [
    ...
    { roleKey: 'role_merchant',    email: 'aissata@miabe.tg',  firstName: 'Aissata' },
  ...
  useEffect(() => {
    if (roleParam && ['farmer', 'merchant', 'transporter'].includes(roleParam)) {
      setRole(roleParam);
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      const map = { farmer: '/agriculteur', merchant: '/commercant', transporter: '/transporteur' };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 min-h-screen">
      {/* LEFT PANEL */}
      <div className="hidden lg:flex flex-col justify-between p-10 xl:p-14 bg-gradient-to-br from-emerald-950 via-emerald-800 to-emerald-600 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Link to="/" className="inline-flex items-center gap-2.5 text-2xl font-bold tracking-tight text-white hover:opacity-90 transition">
            <span className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-emerald-300 shadow-inner">
              <FontAwesomeIcon icon={faSeedling} />
            </span>
            <span>Miabé<strong className="text-emerald-300 font-black">Assimé</strong></span>
          </Link>
        </div>
        ...
        <div className="relative z-10 py-10 flex flex-col justify-center max-w-md">
          ...
          <div className="flex flex-col gap-3.5">
            {[
              ...
              <div key={f.key} className="flex items-center gap-3 text-sm font-medium text-white/90">
        ...
        <div className="relative z-10 bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/10">
          ...
          <div className="flex flex-col gap-2">
            {DEMO.map(d => (
              ...
              >
                <span>{d.firstName} ({t(d.roleKey)})</span>
                <span className="text-white/60 text-3xs font-mono">{d.email}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center justify-center p-6 md:p-12 bg-neutral-50 min-h-screen">
        <div className="w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 border border-neutral-200 shadow-xl flex flex-col gap-6">
          ...
          <div className="lg:hidden text-center">
            <Link to="/" className="inline-flex items-center gap-2 text-xl font-bold text-neutral-900">
              <span className="text-emerald-600">
                <FontAwesomeIcon icon={faSeedling} />
              </span>
              <span>Miabé<strong className="text-emerald-600 font-black">Assimé</strong></span>
            </Link>
          </div>
          ...
          {mode === 'register' && (
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-neutral-700">{t('auth_role')}</label>
                ...
                {ROLES.map(r => {
                  ...
                  return (
                    <button
                      ...
                      id={`role-${r.id}`}
                    >
                      <span className={`text-xl ${isActive ? 'text-emerald-700' : 'text-neutral-500'}`}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-2xl bg-emerald-50/80 border border-emerald-200 shadow-2xs">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-emerald-600 text-white flex items-center justify-center text-sm shadow-xs">
                    <FontAwesomeIcon icon={ROLES.find(r => r.id === role)?.icon || faSeedling} />
                  </div>
                  <div>
                    <div className="text-3xs font-bold text-neutral-400 uppercase tracking-wider">Espace sélectionné</div>
                    <div className="text-xs font-black text-emerald-900">{t(`role_${role}`)}</div>
                  </div>
                </div>
                <div className="flex gap-1">
                  {ROLES.map(r => (
                    <button
                      ...
                      onClick={() => {
                        setRole(r.id);
                        const d = DEMO.find(dm => dm.roleKey === `role_${r.id}`);
                        if (d) setForm(prev => ({ ...prev, email: d.email, password: 'demo123' }));
                      }}
                    >
                      {r.id === 'farmer' ? 'Agri' : r.id === 'merchant' ? 'Comm' : 'Camion'}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {mode === 'register' && (
              <>
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-neutral-700">{t('auth_name')}</label>
                  <input
                    name="name"
                    type="text"
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-neutral-700">{t('auth_phone')}</label>
                  <input
                    name="phone"
                    type="tel"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-sm focus:outline-none focus:border-emerald-600 transition"
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-neutral-700">{t('auth_region')}</label>
                  <select
                    name="region"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-neutral-700">{t('auth_email')}</label>
              <input
                name="email"
                type="email"
                className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-sm focus:outline-none focus:border-emerald-600 transition"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-neutral-700">{t('auth_password')}</label>
              <input
                name="password"
                type="password"
                className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-200 text-sm focus:outline-none focus:border-emerald-600 transition"
            ...
            <button
              ...
              id="auth-submit-btn"
            >
              <FontAwesomeIcon icon={mode === 'login' ? faArrowRight : faCheck} />
              <span>
                {loading ? t('auth_loading') : mode === 'login' ? t('auth_submit_login') : t('auth_submit_register')}
              </span>
            </button>
